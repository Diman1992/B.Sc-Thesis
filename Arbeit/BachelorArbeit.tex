...
\begin{align}
 ...
 &=\frac{G_F V}{\sqrt{2}} f_-(q^2)u_\nu \underbrace{(m_\nu + m_l)}_{=0} (1-\gamma_5)v_l\nonumber \\
 &=0
 \end{align}

\subsection{Fit des Formfaktors $f_+$}
\begin{align}
 \chi^2 = \sum\limits_{i,j=1}^m (\Delta \Gamma_i - g_i(q^2))C^{-1}_{ij}(\Delta \Gamma_j - g_j(q^2))
\end{align}
blabladd
\begin{align}
...
blablaadsf
\begin{align}
 C^{\text{stat}}_{ij} = \sigma^{\text{stat}}_i \sigma^{\text{stat}}_j \cdot \rho^{\text{stat}}_{ij}
\end{align}
blabla
\begin{align}
 C^{\text{sys}}_{ij} = \sigma^{\text{sys}}_i \sigma^{\text{sys}}_j \cdot \rho^{\text{sys}}_{ij}
\end{align}
bddffd
