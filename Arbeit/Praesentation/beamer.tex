...
% 
 ...
 \pause
  ...
  \begin{itemize}
   \visible<2->{\item Fermis Goldene Regel:}\visible<3->{ $\underbrace{\Gamma}_{\text{Rate}} = 2\pi \underbrace{\delta(E_f-E_i)}_{\text{Phasenraum }\Phi}\, \cdot \,{\underbrace{|\langle f|V|i\rangle|}_{\text{Amplitude }M}}^2$}
...
\begin{frame}
 ...
 \visible<5->{&\stackrel{\text{Dirac}}{=}\frac{G_F V_{cs}}{\sqrt{2}} f_-(q^2)\bar u_\nu (m_\nu + m_l) (1-\gamma_5)v_l} 
 \end{align*}
...
\begin{frame}
 ...
 \begin{align*}
 \visible<3->{p_D^\mu &= p_K^\mu + p_l^\mu + p_\nu^\mu \\
 p_D^\mu - p_K^\mu &=: q^\mu := p_l^\mu + p_\nu^\mu }\\
 \visible<4->{\left(p_D^\mu-p_K^\mu\right)^2 &= q^2 =  (p_l^\mu + p_\nu^\mu )^2\\
 m_D^2 + m_K^2 - 2m_DE_K &= q^2 = m_l^2 + m_\nu^2 + E_lE_\nu - |\vec p_l||\vec p_\nu|\cos(\xi)}
\end{align*}
\visible<5->{Hieraus ergeben sich bei abermals vernachlässigbaren Leptonenmassen ($E = |\vec p|$) die Bereichsgrenzen zu
\begin{align*}
...
\begin{itemize}
 ...
 \end{itemize}}
 \visible<4->{\item Parametrisierung durch Pol und Polynomreihe in $z$ mit $|z|\stackrel{!}{<}1$
 \begin{itemize}
...
\begin{frame}
 ...
 \begin{itemize}
   ...
   \visible<3->{\item theoretische Werte $g = \frac{G_F^2 |V_{cs}|^2}{24\pi^3}\int|p_K(q_i^2)|^3 \cdot |f_+(q^2_i)|^2 \mathrm{d} q_i^2$}
 \end{itemize}
\end{frame}
...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \visible<4->{\item Bei $D^0\rightarrow K^- e^+ \nu_e$ gelten entsprechende Einflüsse, nur nicht so deutlich}
 \end{itemize}
\end{frame}
